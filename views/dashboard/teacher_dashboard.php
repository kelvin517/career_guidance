...
// ─── Fetch teacher profile (subject, qualification) ─────────────────────
$teacherProfile = [];
if ($conn) {
...
// ─── Recent signups (for chart trend) ──────────────────────────────────
$recentSignups = [];
$signupRes = mysqli_query($conn, "
...
");
if ($signupRes) {
    while ($row = mysqli_fetch_assoc($signupRes)) {
        $recentSignups[] = $row;
    }
}

// ─── Notifications / messages (unread count + latest) ──────────────────
$unread = 0;
$notifications = [];
if ($conn) {
    ...
    $notifRes = mysqli_query($conn, "
        SELECT m.id, m.subject AS title, m.sent_at AS created_at
        FROM messages m
...
<head>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .notif-btn{position:relative;background:none;border:none;cursor:pointer;font-size:1.1rem;color:var(--ink-soft);padding:8px;border-radius:8px}
        ...
        .mat-type-icon{width:34px;height:34px;background:var(--accent-dim);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:.9rem}
...
<aside class="sidebar">
    ...
    <ul class="sb-nav">
        <li><a href="../messages.php"><span class="nav-icon">✉</span> Messages<?php if ($unread > 0): ?><span class="sb-badge"><?= $unread ?></span><?php endif; ?></a></li>
        <li><a href="../profile.php"><span class="nav-icon">👤</span> Profile</a></li>
    </ul>
    <div class="sb-footer">
        <div class="sb-user">
            <div class="sb-avatar"><?= $avatarLetter ?></div>
            <div><div class="sb-user-name"><?= $fullName ?></div><div class="sb-user-sub">Teacher</div></div>
        </div>
...
<div class="main">
    <div class="topbar">
        ...
        <div class="topbar-right">
            ...
            <button class="notif-btn" title="Notifications">🔔<?php if (!empty($notifications)): ?><span class="notif-dot"></span><?php endif; ?></button>
        </div>
    </div>
    <div class="body">
        <!-- Banner -->
        <div class="banner">
            <div>
                <div class="banner-greeting">Good <?= $greeting ?>, Teacher</div>
                <div class="banner-name"><?= $firstName ?></div>
                <div class="banner-sub">You have <strong style="color:#fff"><?= $publishedQuizzes ?> published <?= $publishedQuizzes === 1 ? 'quiz' : 'quizzes' ?></strong> and <strong style="color:#fff"><?= $totalMaterials ?> learning <?= $totalMaterials === 1 ? 'material' : 'materials' ?></strong> live.</div>
        ...
        <!-- Stats -->
        <div class="stats-row">
            <div class="stat-card"><div class="stat-icon">📝</div><div class="stat-label">My Quizzes</div><div class="stat-value"><?= $totalQuizzes ?></div><div class="stat-delta"><?= $publishedQuizzes ?> published</div></div>
            <div class="stat-card"><div class="stat-icon">📚</div><div class="stat-label">Materials</div><div class="stat-value"><?= $totalMaterials ?></div><div class="stat-delta">Uploaded by you</div></div>
            <div class="stat-card"><div class="stat-icon">✅</div><div class="stat-label">Quiz Attempts</div><div class="stat-value"><?= $totalAttempts ?></div><div class="stat-delta">Across all quizzes</div></div>
            <div class="stat-card"><div class="stat-icon">🎓</div><div class="stat-label">Students (Platform)</div><div class="stat-value"><?= $totalStudents ?></div><div class="stat-delta">Active learners</div></div>
        </div>

        <div class="grid-main">
            <!-- My Quizzes -->
            <div class="panel">
                <div class="panel-head"><span class="panel-title">My Quizzes</span><a href="../quizzes.php" class="panel-link">Manage all →</a></div>
                <div class="panel-body">
                    <?php if ($myQuizzes): ?>
                        <table class="quiz-table">
                            <thead><tr><th>Quiz</th><th>Questions</th><th>Attempts</th><th>Status</th><th></th></tr></thead>
                            <tbody>
                                <?php foreach (array_slice($myQuizzes, 0, 6) as $q): ?>
                                    <tr>
                                        <td><div class="q-title"><?= htmlspecialchars($q['title']) ?></div><div class="q-count"><?= date('d M Y', strtotime($q['created_at'])) ?></div></td>
                                        <td><?= (int)$q['question_count'] ?></td>
                                        <td><?= (int)$q['attempt_count'] ?></td>
                                        <td><span class="pill <?= $q['is_published'] ? 'pill-green' : 'pill-amber' ?>"><?= $q['is_published'] ? 'Live' : 'Draft' ?></span></td>
                                        <td><div class="q-actions"><a href="../quiz_edit.php?id=<?= $q['id'] ?>" class="icon-btn" title="Edit">✏</a><a href="../quiz_results.php?id=<?= $q['id'] ?>" class="icon-btn" title="Results">📊</a></div></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
            ...
            <!-- My Materials -->
            <div class="panel">
                <div class="panel-head"><span class="panel-title">My Materials</span><a href="../materials.php" class="panel-link">View all →</a></div>
                <div class="panel-body">
                    <?php if ($myMaterials): ?>
                        <ul class="mat-list">
                            <?php
                            ...
                            ?>
                                <li><a href="../material_view.php?id=<?= $m['id'] ?>" class="mat-item"><div class="mat-type-icon"><?= $icon ?></div><div><div class="mat-name"><?= htmlspecialchars($m['title']) ?></div><div class="mat-meta"><?= strtoupper($m['type']) ?> · <?= $m['category'] ?? 'General' ?></div></div><div class="mat-views"><?= (int)($m['views'] ?? 0) ?> views</div></a></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
        ...
        <div class="grid-bottom">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <?php if ($myQuizzes): ?>
                        <?php $maxAttempts = max(array_column($myQuizzes, 'attempt_count')) ?: 1; ?>
                        <div class="bar-chart">
                            <?php foreach (array_slice($myQuizzes, 0, 6) as $q): ?>
                                <div class="bc-row">
                                    <div class="bc-label" title="<?= htmlspecialchars($q['title']) ?>"><?= htmlspecialchars(mb_substr($q['title'], 0, 14)) ?></div>
                                    <div class="bc-track"><div class="bc-fill" style="width:<?= round(($q['attempt_count']/$maxAttempts)*100) ?>%"></div></div>
            ...
            <!-- Notifications -->
            <div class="panel">
                <div class="panel-head"><span class="panel-title">Recent Notifications</span><a href="../messages.php" class="panel-link">Inbox →</a></div>
                <div class="panel-body">
                    <?php if ($notifications): ?>
                        <ul class="feed">
                            <?php foreach ($notifications as $n): ?>
                                <li class="feed-item"><div class="feed-dot"></div><div><div class="feed-text"><?= htmlspecialchars($n['title']) ?></div><div class="feed-time"><?= date('d M, H:i', strtotime($n['created_at'])) ?></div></div></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
